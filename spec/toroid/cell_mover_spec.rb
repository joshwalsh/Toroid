...
describe Toroid::CellMover do
  let(:planet) { mock() }
  subject { Toroid::CellMover.new planet }
  ...
  context 'moves organisms' do
    ...
    context 'when not able to move' do
      it 'does not allow moving an organism into an occupied cell' do
        ...
        expect { subject.move(3, 3, :right) }.to raise_error Toroid::CellMover::CurrentlyOccupiedError
      end
    end
    ...
    context 'at random' do
      ...
      it 'moves into neighboring cell when all cells are unoccupied' do
        ...
        Toroid::SelectRandomCoordinate.stub(:from_set) { [3,4] }
